...
def show_stats(total_marks):
    max_mark = max(total_marks)
    min_mark = min(total_marks)
    average_mark = sum(total_marks) / len(total_marks)

    highest_scorers = [(data['Name'], data['StudentID']) for data in students_data.values() if data['Total'] == max_mark]
    lowest_scorers = [(data['Name'], data['StudentID']) for data in students_data.values() if data['Total'] == min_mark]
    fail_count = sum(1 for data in students_data.values() if data['Total'] < 50)

    print(f"Average Mark          : {average_mark:.2f}")
    print(f"Highest Mark          : {max_mark}")
    print(f"Lowest Mark           : {min_mark}")
    print(f"Students with the Highest Mark: {highest_scorers}")
    print(f"Students with the Lowest Mark: {lowest_scorers}")
